...
__global__ void computeMD(int N, int w0, int *d_m1, int *d_m2, int *d_H, int *d_W, int *d_a, int *d_b) {
    int globalThreadIdx = blockIdx.x * blockDim.x + threadIdx.x;
    int h0_start = max(1, -N + w0);
    int h0_end = min(N, w0 - 1);

    for (int h0 = h0_start + globalThreadIdx; h0 <= h0_end; h0 += blockDim.x * gridDim.x) {
        int index = h0 * (N + 2) + (w0 - h0);

        for (int i3 = 1; i3 <= h0; i3++) {
            d_m1[index] = MAX(d_m1[index], d_H[index - i3 * (N + 2)] + d_W[i3]);
        }


        for (int i3 = 1; i3 <= w0 - h0; i3++) {
            d_m2[index] = MAX(d_m2[index], d_H[index - i3] + d_W[i3]);
        }

        d_H[index] = MAX(0, MAX(d_H[index - (N + 2) - 1] + sigma_device(d_a[h0], d_b[h0]),
                              MAX(d_m1[index], d_m2[index])));
    }
}
...
int main() {
    ...
    for (int i = 0; i < (N + 2) * (N + 2); i++) {
        h_m1[i] = INT_MIN;
        h_m2[i] = INT_MIN;
        CPU_H[i] = h_H[i] = rand() % 10;  // Example initialization
    }
...
// traco
	...
	for (int w0 = 2; w0 <= 2 * N; w0 += 1) {
	        computeMD<<<numBlocks, threadsPerBlock>>>(N, w0, d_m1, d_m2, d_H, d_W, d_a, d_b);
        cudaDeviceSynchronize();
    }
  ...
  for (int h0 = max(1, -N + w0); h0 <= min(N, w0 - 1); h0 += 1) {
    {
      ...
      for (int i3 = 1; i3 <= h0; i3 += 1) {
        m1[h0][w0 - h0] = MAX(m1[h0][w0 - h0], H[h0 - i3][w0 - h0] + h_W[i3]);
      }

      for (int i3 = 1; i3 <= w0 - h0; i3 += 1) {
        m2[h0][w0 - h0] = MAX(m2[h0][w0 - h0], H[h0][w0 - h0 - i3] + h_W[i3]);
      }
    }
    H[h0][w0 - h0] = MAX(0, MAX(H[h0 - 1][w0 - h0 - 1] + sigma_host(h_a[h0], h_b[h0]), MAX(m1[h0][w0 - h0], m2[h0][w0 - h0])));
  }
}
...
for (int i=1; i <=N; i++){
    for (int j=1; j <=N; j++){
        for (int k=1; k <=i; k++)
            m1[i][j] = MAX(m1[i][j] ,H[i-k][j] + h_W[k]);
        for (int k=1; k <=j; k++)
            m2[i][j] = MAX(m2[i][j] ,H[i][j-k] + h_W[k]);
        H[i][j] = MAX(0, MAX( H[i-1][j-1] + sigma_host(h_a[i], h_b[i]), MAX(m1[i][j], m2[i][j])));
    }
}
*/
